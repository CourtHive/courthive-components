...
export const LARGE_CATALOG: CatalogMatchUpItem[] = [
  ...
  },
  {
    ...
    isScheduled: true,
    scheduledTime: '11:00',
    scheduledCourtName: 'Court 4',
    sides: femaleSides(4, 6)
  },
  {
    matchUpId: 'M16',
    eventId: 'E2',
...
for (let i = 0; i < 8; i++) {
  SCROLLING_CATALOG.push({
    matchUpId: `X${String(seqId++).padStart(2, '0')}`,
    eventId: 'E10',
    eventName: BOYS_U18_SINGLES,
    drawId: 'D10',
    drawName: 'Main',
    structureId: 'S10',
    roundNumber: 1,
...
for (let i = 0; i < 4; i++) {
  SCROLLING_CATALOG.push({
    ...
    matchUpFormat: FORMAT_STANDARD,
    isScheduled: i < 2,
    scheduledTime: i < 2 ? '10:00' : undefined,
...
export const SAMPLE_ISSUES: ScheduleIssue[] = [
  {
    severity: 'ERROR',
    message: `${MALE_NAMES[0]} has back-to-back matches with no recovery time`,
    matchUpId: 'M01',
    date: DATE_DAY2
  },
  {
    severity: 'WARN',
    message: 'Court 1 has 6 consecutive hours of matches \u2014 consider rest window',
    date: DATE_DAY2
  },
  { severity: 'INFO', message: 'Boys U16 Doubles R16 has no assigned court yet', matchUpId: 'M11', date: DATE_DAY1 },
  {
    severity: 'ERROR',
    message: `${MALE_NAMES[4]} is scheduled on two courts at the same time`,
    matchUpId: 'M03',
    date: DATE_DAY2
  },
...
export function makeMockCourtGrid(courts: number = 6, callbacks?: MockGridCallbacks): MockCourtGrid {
