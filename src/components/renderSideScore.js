...
export function setScore({ gameScoreOnly, scoreStripes, set, sideNumber }) {
  ...
  const variant =
    (isWinningSide && "winner") || set?.winningSide ? "loser" : undefined;
  const gameScore = sideNumber === 2 ? set.side2Score : set.side1Score;
  const tieBreakScore =
    sideNumber === 2 ? set.side2TiebreakScore : set.side1TiebreakScore;
  const tieBreakSet = gameScore === undefined && tieBreakScore;
  const scoreDisplay = tieBreakSet || gameScore;
  const stripedScore =
    scoreStripes && set.setNumber % 2 ? "lightgray" : "transparent";
...
export function renderSideScore({
  ...
  const wrapperHeight =
    sideNumber === 2 ? participantHeight : participantHeight - 1; // account for border!!
