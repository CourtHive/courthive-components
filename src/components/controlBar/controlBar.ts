...
export function controlBar(params: {
  ...
  for (const item of items) {
    ...
    if (itemConfig.location === HEADER) {
      handleHeaderItem(itemConfig, target);
      headerCount += 1;
    }

    const result = processItem(item, itemConfig, location, stateChange, onClick, selectItemFn, elements, inputs);
...
function handleHeaderItem(itemConfig: any, target: HTMLElement): void {
...
function processItem(
  ...
  stateChange: any,
  onClick: (e: any, itemConfig: any) => void,
  selectItemFn: any,
  ...
  if (!itemConfig.hide && (itemConfig.input || itemConfig.placeholder || itemConfig.search)) {
    const inputResult = buildInputItem(item, itemConfig);
    if (itemConfig.id) inputs[itemConfig.id] = inputResult.input;
    if (itemConfig.id) {
      elements[itemConfig.id] = inputResult.elem;
      inputResult.elem.id = itemConfig.id;
    }
    location?.appendChild(inputResult.elem);
    return itemConfig.focus ? { focus: inputResult.input } : undefined;
  }
  ...
  renderButtonItem(itemConfig, location, stateChange, onClick, selectItemFn, elements);
...
function buildInputItem(
  ...
  if (itemConfig.search) {
    appendSearchIcons(elem, input, itemConfig);
  }
...
function appendSearchIcons(elem: HTMLElement, input: HTMLInputElement, itemConfig: any): void {
...
function renderButtonItem(
  ...
  if (itemConfig.options) {
    ...
    renderSelectionItem(itemConfig, location, stateChange, selectItemFn, elements);
  } else if (itemConfig.tabs) {
    renderTabsItem(itemConfig, location, elements);
  } else {
