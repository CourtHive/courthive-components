...
function addRoundRobinFields(
    ...
    relationships.push({
      ...
      onChange: ({ e }: any) => {
        ...
        if (newAdvance > currentAdvance) {
          const lastEdge = positionEdges.at(-1)!;
          const toAdd: number[] = [];
  ...
  if (positionEdges.length > 0 && advanceCount > 0) {
    addPlayoffTargetsSummary(positionEdges, state, groupCount, items);
  }
}

function addPlayoffTargetsSummary(
  positionEdges: TopologyEdge[],
  state: TopologyState,
  groupCount: number,
  items: any[]
): void {
...
function addPlayoffFields(
  ...
  if (!inboundPositionEdge) return;
  const sourceNode = state.nodes.find((n) => n.id === inboundPositionEdge.sourceNodeId);
  if (sourceNode?.structureType !== ROUND_ROBIN) return;
...
export function buildNodeEditor(callbacks: NodeEditorCallbacks): UIPanel<TopologyState> {
  ...
  function update(state: TopologyState): void {
    ...
    if (node.structureType === ROUND_ROBIN) {
      addRoundRobinFields(node, state, isReadOnly, callbacks, items, relationships);
    }

    if (node.stage === PLAY_OFF) {
      addPlayoffFields(node, state, isReadOnly, callbacks, items, relationships);
    }

    if (node.stage === QUALIFYING) {
      addQualifyingFields(node, isReadOnly, callbacks, items, relationships);
    }
    ...
    appendFeedWarnings(node, state, body);
