...
export function buildEdgeEditor(callbacks: EdgeEditorCallbacks): UIPanel<TopologyState> {
    ...
    if (!isReadOnly) {
      if (hasRoundRanges) {
        ...
        relationships.push({
          control: 'sourceRoundNumber',
          onChange: ({ inputs }: any) => {
            const val = Number.parseInt(inputs.sourceRoundNumber.value) || 0;
      ...
      relationships.push({
        ...
        onChange: ({ inputs }: any) => {
          const val = Number.parseInt(inputs.targetRoundNumber.value) || 0;
  ...
  function buildRRPositionCheckboxes(
    edge: TopologyEdge,
    source: any,
    state: TopologyState
  ): HTMLElement {
  ...
  function update(state: TopologyState): void {
    ...
    if (!isRRPosition) {
      const roundFields = buildRoundItems(edge, source, target, sourceMaxRound, targetMaxRound, isQualifyingWinner);
      items.push(...roundFields.items);
      relationships.push(...roundFields.relationships);
    }
    ...
    if (isQualifyingWinner && source) {
      ...
      if (!isReadOnly) {
        relationships.push({
          ...
          onChange: ({ inputs }: any) => {
            const val = Number.parseInt(inputs.qualifyingPositions.value) || 1;
    ...
    if (isRRPosition && source) {
      body.appendChild(buildRRPositionCheckboxes(edge, source, state));
    }
