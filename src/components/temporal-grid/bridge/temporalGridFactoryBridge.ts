/**
 * Temporal Grid Factory Bridge
 * 
 * Translation layer between Temporal Grid Engine and Competition Factory TODS.
 ...
 */
function aggregateAvailabilityByVenue(
  entries: TodsDateAvailability[],
): TodsDateAvailability[] {
...
export function applyTemporalAvailabilityToTournamentRecord(params: {
  ...
  const updated = JSON.parse(JSON.stringify(tournamentRecord));
 ...
 */
export function buildSchedulingProfileFromUISelections(
...
export function todsAvailabilityToBlocks(params: {
  ...
  const venueId = venue.venueId || venue.venueName || 'unknown-venue';
    ...
    if (courtIds && courtIds.length > 0) {
      ...
      if (venue.courts && venue.courts.length > 0) {
        for (const court of venue.courts) {
          const courtId = court.courtId || court.courtName || 'unknown-court';
          blocks.push({
            court: {
              tournamentId,
              facilityId: venueId,
 ...
 */
export function mergeOverlappingAvailability(
  entries: TodsDateAvailability[],
): TodsDateAvailability[] {
