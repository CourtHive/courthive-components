 ...
 * Conflict Evaluators
 * 
 * Pluggable conflict detection system for the Temporal Grid Engine.
  ...
  evaluate: (_ctx: EngineContext, mutations: BlockMutation[]): EngineConflict[] => {
    ...
    for (const mutation of mutations) {
      ...
      if (blockEndTime > sunsetTime) {
        // In a real implementation, we'd check court metadata here
...
export const createFollowByEvaluator = (_factoryAPI: any) => ({
  ...
  evaluate: (_ctx: EngineContext, _mutations: BlockMutation[]): EngineConflict[] => {
    ...
    // 3. Call factoryAPI.proConflicts(previewRecord, options)
    // 4. Map results to EngineConflict[]
