...
export const POLICY_TYPE_METADATA: PolicyTypeMeta[] = [
  ...
  meta(POLICY_TYPE_SCHEDULING, 'Scheduling', 'Match scheduling, average times, recovery times, daily limits', GROUP_TOURNAMENT_OPS, true),
  meta(POLICY_TYPE_POSITION_ACTIONS, 'Position Actions', 'Allowed position actions (alternates, walkovers, withdrawals)', GROUP_TOURNAMENT_OPS, false),
  meta(POLICY_TYPE_MATCHUP_ACTIONS, 'MatchUp Actions', 'Allowed matchUp status transitions and actions', GROUP_TOURNAMENT_OPS, false),
  meta(POLICY_TYPE_AVOIDANCE, 'Avoidance', 'Draw placement avoidance rules (club, nationality, region)', GROUP_TOURNAMENT_OPS, false),
  ...
  meta(POLICY_TYPE_VOLUNTARY_CONSOLATION, 'Voluntary Consolation', 'Voluntary consolation entry rules', GROUP_DRAW_CONFIG, false),
  meta(POLICY_TYPE_ROUND_NAMING, 'Round Naming', 'Custom round name labels (QF, SF, F, etc.)', GROUP_DRAW_CONFIG, false),

  // Participants
  meta(POLICY_TYPE_PARTICIPANT, 'Participant', 'Participant display and data rules', GROUP_PARTICIPANTS, false),

  // Display & Audit
  meta(POLICY_TYPE_DISPLAY, 'Display', 'Client display configuration (public vs admin views)', GROUP_DISPLAY_AUDIT, false),
