...
export function buildAggregationSection(store: RankingPointsEditorStore): {
  ...
  function buildTiebreakSection(criteria: string[], readonly: boolean): void {
  ...
  function buildBucketCard(bucket: any, bi: number, readonly: boolean): HTMLElement {
    ...
    if (readonly && bucketFields.length) {
  ...
  function rebuild(state: RankingPointsEditorState): void {
    ...
    for (const [field, label, type, value, options] of scalarFields) {
      ...
      container.appendChild(buildScalarField(field, label, type, value, options, state.readonly));
    }
    ...
    buildTiebreakSection(rules?.tiebreakCriteria ?? [], state.readonly);
    ...
    if (buckets.length || !state.readonly) {
      ...
      for (let bi = 0; bi < buckets.length; bi++) {
        container.appendChild(buildBucketCard(buckets[bi], bi, state.readonly));
      }
