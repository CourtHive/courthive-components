...
export function openCompositionEditorModal(options: CompositionEditorOptions = {}) {
  ...
  const buildFooterContent = (container: HTMLElement) => {
    ...
    footerInputs = renderForm(container, [
      ...
      { divider: true },
      { field: 'showPageNumbers', label: 'Show Page Numbers', checkbox: true, checked: fi.showPageNumbers !== false },
      { field: 'showTimestamp', label: 'Show Timestamp', checkbox: true, checked: fi.showTimestamp !== false },
      { divider: true },
