...
export function renderDialPadScoreEntry(params: RenderScoreEntryParams): void {
  ...
  try {
    ...
    if (internalScore) {
      ...
      const initialScoreString = formatScoreString(state.digits, {
        matchUpFormat: matchUp.matchUpFormat || DEFAULT_FORMAT
      });
    ...
    if (isAggregateScoring) {
      ...
      aggregateIndicator.style.backgroundColor = CHC_BG_SECONDARY;
    ...
    irregularEndingContainer.style.backgroundColor = CHC_BG_SECONDARY;
    ...
    const updateMatchUpDisplay = (outcome: ScoreOutcome | null | { clearAll?: boolean }) => {
      ...
      const displayMatchUp = {
        ...matchUp,
        score: (outcome as any)?.clearAll ? undefined : hasOutcomeScore ? internalScore : undefined,
