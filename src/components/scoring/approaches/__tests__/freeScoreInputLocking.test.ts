...
function shouldLockInput(currentValue: string, previouslyLocked: boolean, matchUpFormat: any): boolean {
  ...
  if (!previouslyLocked && scoreString) {
    ...
    const endsWithSeparator = rawValue.length > 0 && /[\s-]$/.test(rawValue);
    return scoreString.length > 0 && endsWithSeparator; // Stay locked if has content and separator, otherwise unlock
  }
...
describe('freeScore Input Locking Logic', () => {
  ...
  describe('Best-of-5 Matches', () => {
    ...
    it('should lock after 3-2 best-of-5 match', () => {
      const inputs = ['64', '64 46', '64 46 63', '64 46 63 36', '64 46 63 36 75', '64 46 63 36 75 '];
      const lockStates = simulateInputSequence(inputs, SET5_FORMAT);
