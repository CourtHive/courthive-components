 ...
 *
 * Tests for venue-level availability support in the temporal grid engine:
...
import { venueKey, venueDayKey } from '../../components/temporal-grid/engine/railDerivation';
import { BLOCK_TYPES } from '../../components/temporal-grid/engine/types';
...
describe('Court + Venue intersection logic', () => {
  let engine: TemporalGridEngine;

  beforeEach(() => {
    engine = new TemporalGridEngine();
...
describe('Snapshot preservation', () => {
  it('venue data survives createSnapshot via simulateBlocks', () => {
    ...
    const result = engine.simulateBlocks([]);
