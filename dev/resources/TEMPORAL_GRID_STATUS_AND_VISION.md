...
- 🔒 **CLOSED** - Court closed during normally open hours
...
type BlockType = 
  ...
  | 'CLOSED'         // Closed during open hours
...
1. Start with court's open hours as base AVAILABLE segment
...
2. Find next painted block after clicked time
...
describe('Rail Derivation - Inverted Paradigm', () => {
  test('no blocks = fully available during open hours', () => {
    ...
    expect(segments).toEqual([
      { start: '06:00', end: '22:00', status: 'AVAILABLE' }
    ]);
  });
  ...
  test('maintenance block creates availability gaps', () => {
    const court = { openTime: '06:00', closeTime: '22:00' };
    const blocks = [
      { start: '08:00', end: '09:00', type: 'MAINTENANCE' }
    ];
    const segments = deriveRailSegments(court, blocks);
    expect(segments).toEqual([
  ...
  test('scheduled blocks reduce available time', () => {
    const blocks = [
      { start: '14:00', end: '16:00', type: 'SCHEDULED' }
    ];
    const segments = deriveRailSegments(court, blocks);
    const availableTime = segments
